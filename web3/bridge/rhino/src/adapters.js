...
import { Client, Config, Signer } from '@paradex/sdk';
 ...
 * - For Paradex, creates an account from the private key, then returns a Paradex adapter.
 ...
 */
export async function chainAdapter(_fromChainType, _fromAddress, _fromPrvKey, _chainConfig) {
    ...
    if (_fromChainType === ChainType.PARADEX) {
        const ethersWallet = new ethers.Wallet(_fromPrvKey);
        const config = await Config.fetch('prod');
        const signer = Signer.fromEthers(ethersWallet);

        const paradexClient = await Client.fromEthSigner({
            config,
            signer
        });

        return getParadexChainAdapterFromAccount(paradexClient, _chainConfig);
    }
